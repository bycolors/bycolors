...
void ofApp::setup(){
    ...
    tPhotoGridFbo.allocate(3892, 3892,GL_RGBA, 8);
...
//    AnimCurve curve2 = (AnimCurve) (CUBIC_EASE_IN);
    ...
    tPhotoGridX.animateFromTo((-allGridLocators[detectedColor-1].x*screenHeight/1080), 0);
    tPhotoGridY.animateFromTo((-allGridLocators[detectedColor-1].y*screenHeight/1080), 0);
    
    tPhotoGridPos.setPosition(ofPoint((-allGridLocators[detectedColor-1].x*screenHeight/1080),(-allGridLocators[detectedColor-1].y*screenHeight/1080)));
    tPhotoGridPos.animateTo(ofPoint(0,0));

    tPhotoGridSize.animateFromTo(tPhotoGridFbo.getHeight()*screenHeight/1080, screenHeight);

    tPhotoGridPos.setDuration(2);
    tPhotoGridX.setDuration(2);
    tPhotoGridY.setDuration(2);
    tPhotoGridSize.setDuration(2);

    AnimCurve curve3 = (AnimCurve) (EASE_IN_EASE_OUT);

    tPhotoGridPos.setCurve( curve3 );
    tPhotoGridX.setCurve( curve3 );
    tPhotoGridY.setCurve( curve3 );
    tPhotoGridSize.setCurve( curve3 );
...
void ofApp::update(){
    switch (state) {
        ...
        case STATE_TPHOTO_MONO_IN_2:
            ...
            if(fadeMaskAlpha<255){
                ...
                if(frameCounter<tPhotoMono2HoldTime) {
                    ...
                    //preload next scene
                    tPhotoGridFbo.begin();
                    tPhotoGridImg.draw(0,0,tPhotoGridFbo.getWidth(),tPhotoGridFbo.getHeight());
                    tPhotoMonoImg.draw(allGridLocators[detectedColor-1].x,allGridLocators[detectedColor-1].y);
                    tPhotoGridFbo.end();
                }
        ...
        case STATE_TPHOTO_GRID:
            if(!tPhotoGridPos.hasFinishedAnimating()){
                float dt = 1.0f / 60.0f;
                tPhotoGridPos.update( dt );
                tPhotoGridX.update( dt );
                tPhotoGridY.update( dt );
                tPhotoGridSize.update( dt );
        ...
        case STATE_TCOLOR_FILL:
            ...
            if(!animatedCircleSize.hasFinishedAnimating()){
                ...
                ofTranslate(tPhotoGridFbo.getWidth()/2,tPhotoGridFbo.getHeight()/2); // using 0 starting numbering system
...
void ofApp::draw(){
    ...
    switch (state) {
        ...
        case STATE_TPHOTO_GRID:
            //show taiwan photo info, FADE in;
            ofPushMatrix();
            ofTranslate((screenWidth-screenHeight)/2+canvasOffsetX, canvasOffsetY);
            tPhotoGridFbo.draw(tPhotoGridPos.getCurrentPosition().x,tPhotoGridPos.getCurrentPosition().y,tPhotoGridSize,tPhotoGridSize);
            ofPopMatrix();
